...
#######################
echo "设置host文件"
echo "${outputs.mysql_instance.privateIp}    "  `hostname` >>/etc/hosts
...
chown -R mysql:mysql ${MYSQLBACKUPPATH}
...
wsrep_cluster_address=gcomm://${outputs.mysql_instance.privateIp}
wsrep_node_address=${outputs.mysql_instance.privateIp}
...
echo "配置root密码"
mysql -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '${MYSQLROOTPASSWD}';"
echo "配置sst密码"
mysql -e "GRANT RELOAD, LOCK TABLES, REPLICATION CLIENT ON *.* TO 'sst'@'localhost' IDENTIFIED BY '${MYSQLSSTPASSWD}';"


echo "准备备份脚本"

cat <<"EOF" >${MYSQLDATAPATH}/scripts/backup_delete.sh
#! /bin/sh
backupday=`date +%Y%m%d%H%M%S`
deleteday=`date -d "${MYSQLBACKUPKEEP} days ago" +%Y%m%ds`
backupfile="${MYSQLBACKUPPATH}/`hostname`-mysql-${backupday}.tar"
if innobackupex --defaults-file="/etc/my.cnf" --user="sst" --password="${MYSQLSSTPASSWD}" --no-version-check --stream=tar /tmp | xz -T2 -1 - > "${backupfile}" 2>/dev/null;then
  rm -rf ${MYSQLBACKUPPATH}/`hostname`-mysql-${deleteday}*.tar
fi
EOF
chmod +x ${MYSQLDATAPATH}/scripts/backup_delete.sh

echo "0 1 */1 * * ${MYSQLDATAPATH}/scripts/backup_delete.sh" >> /var/spool/cron/root
